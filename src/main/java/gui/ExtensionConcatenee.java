...
public class ExtensionConcatenee extends ExtensionJAP {
    ...
    ExtensionConcatenee(int id){
        /*try {
            this.id=id;
            Connection connect = com.pca.gui.ApplicationPM.DB.getConnect();//
            Statement statement = connect.createStatement();
            String sql="SELECT * FROM `" + com.pca.gui.ApplicationPM.DB.getTable_ExtensionConcatenee() + "` where id="+id;
    ...
    void save(){
        /*try{
            ...
            String sql2 = "select max(id) as max from  " + com.pca.gui.ApplicationPM.DB.getTable_ExtensionConcatenee();
            ResultSet rs = statement.executeQuery(sql2);
            if (rs.next()) {
                this.id=rs.getInt("max")+1 ;
            }
            String sql = "insert into `" + com.pca.gui.ApplicationPM.DB.getTable_ExtensionConcatenee() + "`(id,idPCA, extensionJap_id,nomTableauBulbapedia,premierNumero)  values (?,?,?,?,?)";
